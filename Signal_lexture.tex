...
Considering two functions $f(x)$ and $g(x)$ with Fourier transform $F(t)$ and $G(t)$. We define an operation
\begin{eqnarray}
f\ast g = \int_{-\infty}^{\infty}g(y)f(x-y)dy
\label{eq:convolution}
\end{eqnarray}
as convolution of the two functions $f(x)$ and $g(x)$ over the interval $\left{-\infty ~ \infty \right}$. It satisfies the following relation:
\begin{eqnarray}
f \ast g = \int_{-\infty}^{\infty}F(t)G(t) e^{itx}dt
